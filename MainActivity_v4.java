...
public class MainActivity extends Activity {
	...
	//Here we will will be declaring all the necessary global variables!
    ...
    class calculate implements View.OnClickListener
    {
        ...
        void updatenumber()
        {
            ...
            if(tracker == 'd')
            {
                ...
                else if (function != '@' && num.length() == 0)
                {
                    num = "0";
                    number = num;
                    commaNumD = String.valueOf(first);
					commaNumC = num;
					if(commaNumD.length() >= 4)
					{
						commaNumD = addComma(String.valueOf(commaNumD));
					}
					if(commaNumC.length() >= 4)
					{
						commaNumC = addComma(commaNumC);
					}
					fullCalc = commaNumD + " " + function + " " + commaNumC;
                ...
                else
				{
					...
					{
						commaNumC = addComma(commaNumC);
						outputnumber.setText(commaNumC);
					}
					else
					{
					...
					case '+':
						if (precalc) {
							if ((outputnumber.getText() != "0") && precalc)
								anstracker = true;
							commaNumA = String.valueOf(ans);
							if(commaNumA.length() >= 4)
							{
								commaNumB = addComma(commaNumA);
								fullCalc = commaNumB + " + ";
							}
							else
							{
					...
					case '=':
						if(first == 0 && second == 0 && result == 0)
						{
							...
							if (compute(number) == -1)
								...
								commaNumA = finalresult;
								if(commaNumA.length() >= 4)
								{
									commaNumB = addComma(commaNumA);
									finalresult = commaNumB;
								}
								else
								{
							...
							/*else if (round.length() > 11)
							 ...
							 break;
								...
								if(fullCalc.length() > 20)
								{
									...
									{
										commaNumB = addComma(String.valueOf(result));
										finalresult = commaNumB;
									}
									else
									{
        ...
        String addComma(String input)
        {
          char[] noCommas = input.toCharArray();
		  boolean oneless;
		  commaCounter = noCommas.length / 3;
		  char[] withCommas;
		  if((noCommas.length % 3) == 0)
		  {
			  commaCounter -= 1;
			  withCommas = new char[noCommas.length + commaCounter];
			  oneless = true;
		  }
		  else
		  {
		  ...
		  CommaCalculator(noCommas.length, 0, 0, noCommas, withCommas, oneless, commaCounter);
		  
		  return commaNumA;
        }
		
		void CommaCalculator(int r, int p, int pp, char[] thedata, char[] newdata, boolean oneless, int counter)
		{
			if(r == 0)
			{
				commaNumA = String.valueOf(newdata);
				return;
			}
			r--;
			...
			if(p == 3)
			{
				...
				{
					pp = pp;
					newdata[(r - pp) + counter] = thedata[r];
					p = 0;
				}
				else
				{
					newdata[(r - pp) + counter] = thedata[r];
					newdata[((r - pp) - 1) + counter] = ',';
					pp++;
					p = 0;
					System.out.println("r = : " + r);
				}
			...
			CommaCalculator(r,p,pp, thedata, newdata, oneless, counter);
		}
